...
/* 
 * Prints all options to outpur stream
 * [Added by MF, May 2009]
 */
void 
AnyOption::printOptions(std::ofstream& out)
{
	if( !valueStoreOK() )
		return;
	for( int i = 0 ; i < option_counter ; i++ ){
    char* val = values[ optionindex[i] ];
    if (val != NULL)
      out << options[i] << "\t" << val << endl;
    // Check values AND flags
//     if( strcmp( TRUE_FLAG , val ) == 0 ) // Flag set 
//       out << optcharindex[i] << endl;
//     else {
//       out << optcharindex[i] << " = " << string(val) << std::endl;
//     }
	}
}
